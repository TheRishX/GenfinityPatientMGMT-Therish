...
function appointmentEmail(patient: Patient, date: string, time: string): { subject: string; text: string; html: string } {
  ...
  // Use the public Cloudinary asset for outbound email. Many email providers
  // cannot render data-URI or server-local image sources after delivery.
  const logo = appointmentEmailLogoUrl;
