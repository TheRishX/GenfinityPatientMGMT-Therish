...
export default function CommunicationsView({ patients, claims }: CommunicationsViewProps) {
  ...
  return <div className="space-y-6 animate-fade-in pb-16">
    ...
    {activeSection === 'email' ? <section className="workspace-section"><div className="flex items-center justify-between mb-4"><div><h3 className="section-heading"><span className="material-symbols-outlined text-primary">forward_to_inbox</span>Choose a patient</h3><p className="text-xs text-on-surface-variant mt-1">Start with a ready-made template, then personalize before sending.</p></div><span className="status-badge">{emailPatients.length} with email</span></div><div className="divide-y divide-surface-container-highest/30">{emailPatients.map(patient => <div key={patient.id} className="py-3 flex items-center justify-between gap-4"><div className="min-w-0"><p className="font-extrabold text-on-surface truncate">{patient.name}</p><p className="text-xs text-on-surface-variant truncate">{patient.email} · {patient.careStage || patient.status}</p></div><button onClick={() => setEmailPatient(patient)} className="secondary-button shrink-0"><span className="material-symbols-outlined text-sm">edit_note</span>Compose</button></div>)}{emailPatients.length === 0 && <p className="py-10 text-center text-sm text-on-surface-variant">No patients with email addresses match this search.</p>}</div></section> : <section className="workspace-section"><div className="flex items-center justify-between mb-4"><div><h3 className="section-heading"><span className="material-symbols-outlined text-primary">receipt_long</span>Invoice receipts</h3><p className="text-xs text-on-surface-variant mt-1">Every receipt is filled from the patient profile and includes the Genfinity branding.</p></div><span className="status-badge">{filteredClaims.length} invoices</span></div><div className="divide-y divide-surface-container-highest/30">{filteredClaims.map(claim => <div key={claim.id} className="py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div><p className="font-extrabold text-on-surface">{claim.claimNumber} · {claim.patientName}</p><p className="text-xs text-on-surface-variant mt-1">{claim.date} · {claim.payer} · ${claim.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p></div><div className="flex gap-2"><a href={`/api/claims/${claim.id}/invoice.pdf`} download={`${claim.claimNumber}.pdf`} className="secondary-button">Download PDF</a><button onClick={() => resendInvoice(claim)} disabled={sendingId === claim.id} className="primary-button">{sendingId === claim.id ? 'Sending...' : 'Email invoice'}</button></div></div>)}{filteredClaims.length === 0 && <p className="py-10 text-center text-sm text-on-surface-variant">No invoices match this search.</p>}</div></section>}
