...
interface Props { patient: Patient; onClose: () => void; onSent?: () => void; }
...
export default function PatientSmsModal({ patient, onClose, onSent }: Props) {
  ...
  const variables = useMemo(() => ({ patientName: patient.name, clinicName: 'Genfinity O&P', appointmentType: patient.deviceCategory || 'appointment' }), [patient]);
